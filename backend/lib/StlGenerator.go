...
func GenerateSTLMapFromSideMap(sideMap []float32, sizeInMM uint32, heightFactor float32, fileName string) error {
	...
	return GenerateSTLMapFromHeightMap(heightMap, sizeInMM, heightFactor, fileName)

}
func GenerateSettlerOfCatan(heightMap [][]float32, sizeInMM uint32, heightFactor float32, fileName string) error {
	var offset int
	var triangles []triangle
	var stepX float32
	var stepY float32
	...
	return generateSTLMapFromTriangles(triangles, fileName)
}
...
func GenerateSTLMapFromHeightMap(heightMap [][]float32, sizeInMM uint32, heightFactor float32, fileName string) error {
	var stepX float32
	var stepY float32
	var sizeX float32
	var sizeY float32
...
func generateSTLMapFromTriangles(triangles []triangle, fileName string) error {
	var header [80]byte
	var byteStl []byte
	byteStl = header[:80]
	...
	err := writeByteToFile(byteStl, fileName)
	return err
}
...
func writeByteToFile(b []byte, fileName string) error {
